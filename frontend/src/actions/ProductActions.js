...
export const getProduct= (keyword="",currentPage=1,price,category) => async (dispatch)=>{
  try {
     ...
     if(price){
      link = `/api/v2/products?keyword=${keyword}&page=${currentPage}&price[lte]=${price[1]}&price[gte]=${price[0]}`;
     }
     if(category && price){
      link = `/api/v2/products?keyword=${keyword}&page=${currentPage}&category=${category}&price[lte]=${price[1]}&price[gte]=${price[0]}`;
     }
